...
If you want to change the prompts used during training, please modify the [`formatting_func_.+` function](./run/src/data_preprocessing.py#L44-L107) in [`data_preprocessing.py`](./run/src/data_preprocessing.py). The following function is a sample for Cookpad.
...
An example of a dataset with the [`formatting_func_cookpad` function](./run/src/data_preprocessing.py#L44-L55C) applied is shown below.
